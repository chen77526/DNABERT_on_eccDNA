...
python run_finetune.py --model_type [dnalongcat/dna] --tokenizer_name=dna$KMER --model_name_or_path $MODEL_PATH --task_name dnaprom --do_predict --data_dir $DATA_PATH  --max_seq_length 1024 --per_gpu_pred_batch_size=128 --output_dir $MODEL_PATH --predict_dir $PREDICTION_PATH --n_process 48 --overwrite_cache --train_type [dataset name] --test_type [testing dataset name]
...
#### Output file: result_[dataset name]_[testing dataset name].tsv
