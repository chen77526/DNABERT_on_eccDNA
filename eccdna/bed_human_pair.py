...
def mainf():
    ...
    with open('./genome/human/hg38_noalt_gap.bed', 'r') as gap_f:
        with open('./db/cell_lines/{}_circleseq_eccdna_filt_uniq.bed'.format(args.cellline), 'r') as input_f:
            with open('./db/cell_lines/{}_circleseq_eccdna_filt_uniq_seq_{}.bed'.format(args.cellline, args.extend * 2), 'w') as output_f1:
                with open('./db/cell_lines/{}_circleseq_eccdna_filt_uniq_seq_pair_{}.bed'.format(args.cellline, args.extend * 2), 'w') as output_f2:
                    with open('./db/cell_lines/{}_circleseq_eccdna_filt_uniq_excl_pair_{}.bed'.format(args.cellline, args.extend * 2), 'w') as output_ex:
                        ...
                        for line in input_f:
                            ...
                            ### save each breakpoint into positive label .bed & save pairs of breakpoints into positive label _pair.bed
                            else:
                                output_f1.writelines('%s\t%d\t%d\n' % (name, int(start)-args.extend, int(start)+args.extend))
                                output_f1.writelines('%s\t%d\t%d\n' % (name, int(end)-args.extend, int(end)+args.extend))
                                output_f2.writelines('%s\t%d\t%d\t%s\t%d\t%d\n' % (name, int(start)-args.extend, int(start)+args.extend, name, int(end)-args.extend, int(end)+args.extend))
