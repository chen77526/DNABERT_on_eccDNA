...
export ext=256                                                          # number of bp extended from center
export seq_len=512                                                      # sequence length
export cellline=PC-3                                                    # name of cellline
...
export eccdna_dir=eccdna_${cellline}_break                              # eccdna directory name
...
python bed_human_break.py --extend $ext --cellline $cellline

### Generate negative label .bed file
bedtools shuffle -i ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_seq_${seq_len}_break.bed -g ./genome/$species/$geno_bound -excl ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_excl_${seq_len}_break.bed > ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_comp_${seq_len}_break.bed

### Check if negative label .bed is intersect with positive label .bed
bedtools intersect -a ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_comp_${seq_len}_break.bed -b ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_excl_${seq_len}_break.bed

### Acquire its corresponding FASTA file
bedtools getfasta -fi ./genome/$species/$geno_ref -bed ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_seq_${seq_len}_break.bed -fo ./output/$species/${species}_${cellline}_positive_${seq_len}_break.fa.out
bedtools getfasta -fi ./genome/$species/$geno_ref -bed ./db/$db_dir/${cellline}_circleseq_eccdna_filt_uniq_comp_${seq_len}_break.bed -fo ./output/$species/${species}_${cellline}_negative_${seq_len}_break.fa.out
